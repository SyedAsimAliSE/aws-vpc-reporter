...
class DirectConnectVIFOperations:
    ...
    def get_virtual_interfaces(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Failed to get Direct Connect VIFs (may not have permissions): {e}")
            return {
                "total_count": 0,
                "virtual_interfaces": [],
                "error": str(e),
        ...
        for vif in virtual_interfaces:
            ...
            for peer in vif.get("bgpPeers", []):
                ...
                bgp_status_summary[bgp_status] = bgp_status_summary.get(bgp_status, 0) + 1
            ...
            processed_vifs.append({
                ...
                "all_bgp_sessions_up": bgp_status_summary.get("up", 0) == len(bgp_peers) and len(bgp_peers) > 0,
                # Additional fields
