...
class NetworkInterfaceOperations:
    ...
    def get_network_interfaces(self, vpc_id: str) -> dict[str, Any]:
        ...
        for eni in network_interfaces:
            ...
            # Parse IPv4 prefixes
            ipv4_prefixes = [prefix.get("Ipv4Prefix") for prefix in eni.get("Ipv4Prefixes", [])]

            # Parse IPv6 prefixes
            ipv6_prefixes = [prefix.get("Ipv6Prefix") for prefix in eni.get("Ipv6Prefixes", [])]
            ...
            processed_enis.append({
                ...
                "attached_to": attachment_info.get("instance_id") if attachment_info else None,
                "is_attached": bool(attachment_info),
                # Public IP association
                "association": association_info,
                "has_public_ip": bool(association_info and association_info.get("public_ip")),
                "public_ip": association_info.get("public_ip") if association_info else None,
                # IP addresses
        ...
        return {
            ...
            "aws_managed_count": len([e for e in processed_enis if e["requester_managed"]]),
            "user_managed_count": len([e for e in processed_enis if not e["requester_managed"]]),
